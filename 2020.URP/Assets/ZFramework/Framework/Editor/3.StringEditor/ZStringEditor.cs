...
namespace ZFramework
{
    public class ZStringEditor
    {
        ...
        [MenuItem("ZFramework/Editor/3.Tool/文件首字母转大写")]
        ...
        [MenuItem("ZFramework/Editor/3.Tool/根据包名设置assetbundle名字")]
        static private void setBundleName()
        {
           ...
           Object[] selects = Selection.objects;
            foreach (Object selected in selects)
            {
                ...
                {
                    if (files[m].ToString().Contains("meta")) continue;
                    string name = files[m].FullName.Replace(files[m].FullName.Substring(0, files[m].FullName.IndexOf("Assets", 0)), "");
                    AssetImporter assetImporter = AssetImporter.GetAtPath(name);
                    assetImporter.assetBundleName = direction.Name;
